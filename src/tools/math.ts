...
export function evaluateCircleSpot(cx, cy, r, theta, type: CircleTypes): Point {
  ...
  const y = Math.round(
    cy - (type === CircleTypes.VerticalEllipse ? 0.5 * r : r) * Math.sin(theta),
  );
...
export function drawCircle(
  ...
  for (let theta = 0; theta < PI2; theta += step) {
    const spot = evaluateCircleSpot(cx, cy, r, theta, type);

    spots.push(spot);

    if (!fill) {
      // write spots
      map.safeWrite(spot.x, spot.y, terrain);
    }
  }
  ...
  if (fill) {
    ...
    for (let spot1 of spots) {
      for (let spot2 of spots) {
        drawLineToSpot(map, spot1.x, spot1.y, spot2.x, spot2.y, terrain);
      }
    }
  }
}
